...
function registrarAuditoria(string $acao,?string $entidade=null,$entidadeId=null,array $detalhes=[]): void { if(!function_exists('db'))return;$ator=(int)($_SESSION['admin_original_id']??$_SESSION['usuario_id']??0)?:null;$efetivo=(int)($_SESSION['usuario_id']??0)?:null;$ids=array_values(array_unique(array_filter([$ator,$efetivo])));$identidades=[];if($ids){$lista=implode(',',array_map('intval',$ids));$res=db()->query("SELECT id,nome,nome_usuario FROM usuarios WHERE id IN ($lista)");foreach($res->fetch_all(MYSQLI_ASSOC) as $p)$identidades[(string)$p['id']]=['nome'=>$p['nome'],'login'=>$p['nome_usuario']];}$detalhes['_identidades']=['autor'=>$ator&&isset($identidades[(string)$ator])?$identidades[(string)$ator]:null,'conta'=>$efetivo&&isset($identidades[(string)$efetivo])?$identidades[(string)$efetivo]:null];$json=json_encode($detalhes,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);$ip=$_SERVER['REMOTE_ADDR']??null;$stmt=db()->prepare('INSERT INTO auditoria(usuario_id,usuario_efetivo_id,acao,entidade,entidade_id,detalhes,ip) VALUES(?,?,?,?,?,?,?)');$eid=$entidadeId===null?null:(string)$entidadeId;$stmt->bind_param('iisssss',$ator,$efetivo,$acao,$entidade,$eid,$json,$ip);$stmt->execute(); }
